	...
	return (
		<div className="container mx-auto px-4 py-12">
			<div className="mb-8 md:mb-12">
				<h1 className="text-3xl sm:text-4xl font-bold mb-4 text-center">Species Dictionary</h1>
				<p className="text-lg sm:text-xl text-muted-foreground mb-8 text-center">Explore our comprehensive database of aquatic species</p>
				...
				<div className="relative w-full max-w-5xl mx-auto mb-12">
					<div className="relative h-[300px] sm:h-[350px] lg:h-[400px] flex items-center justify-center">
						...
						<div className="relative z-10 w-32 h-32 sm:w-64 sm:h-64 lg:w-80 lg:h-80 rounded-full bg-background border-4 border-primary/20 shadow-2xl flex items-center justify-center overflow-hidden">
							<img src={speciesIcon} alt="Species Dictionary" className="w-32 h-32 sm:w-48 sm:h-48 lg:w-56 lg:h-56 object-contain" />
						</div>
				...
				{selectedCategory && (
					<div className="text-center mt-4">
						<Badge variant="secondary" className="text-sm">
							...
							<button onClick={() => setSelectedCategory(null)} className="ml-2 hover:text-destructive">
								×
							</button>
						</Badge>
					</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
				{isLoading ? (
					...
					speciesArray.map((fish) => (
						<Card key={fish.fish_id} className="hover:shadow-lg transition-all hover:-translate-y-1 cursor-pointer">
							<CardHeader>
								<div className="text-5xl mb-4 text-center">
									{/* Optionally render an icon or image */}
									{fish.primary_image ? (
										<img src={fish.primary_image} alt={fish.common_name} className="mx-auto h-16 w-16 object-cover rounded-full" />
									) : (
								...
								<CardTitle className="text-center">{fish.common_name}</CardTitle>
							...
							<CardContent className="space-y-2">
								<div className="flex justify-between items-center text-sm">
									...
									<Badge className={getCareColor(fish.care_level || "")}>{fish.care_level}</Badge>
								</div>
								<div className="flex justify-between text-sm">
									<span className="text-muted-foreground">Size:</span>
									<span className="font-medium">{fish.max_size_cm ? `${fish.max_size_cm} cm` : "N/A"}</span>
								</div>
								<div className="flex justify-between text-sm">
