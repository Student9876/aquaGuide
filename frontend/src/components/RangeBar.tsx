...
const RangeBar: React.FC<RangeBarProps> = ({label, rangeMin, rangeMax, unit, decimals = 1, maxTicks = 10, fixedScale, gradient}) => {
	const format = (v?: number) => (typeof v === "number" && !isNaN(v) ? v.toFixed(decimals) : "N/A");
	const hasRange = typeof rangeMin === "number" && typeof rangeMax === "number" && !isNaN(rangeMin) && !isNaN(rangeMax);
	...
	if (hasRange) {
		...
		if (ticks[ticks.length - 1] !== max) ticks.push(Number(max.toFixed(decimals)));
	}
	...
	return (
		<div className="mb-6">
			...
			{!hasRange && <div className="text-center text-xs text-muted-foreground mt-1">N/A</div>}
		</div>
	);
};
